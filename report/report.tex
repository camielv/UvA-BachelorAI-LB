...
First we count the frequency of each word, i.e. we count how many times it occures in the training set. We also keep track of how many times this word was encountered in either class. For example, if we want to decide whether messages are neutral or not, we keep track of how many times each word has been seen in a neutral message. The number of times the word occurs in the opposite class, is ofcourse equivalent to the total number of encouners, minus the encouters in the first class.
...
Now that every sentence has an associated probability, the real machine learning can begin. The goal is to find a treshold for the probabilities, i.e.\ when a sentence probability is higher than a certain amount, we classify it as $C_1$,  otherwise, we classify it as belonging to $C_2$.
