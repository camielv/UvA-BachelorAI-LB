...
Our dataset was provided by our client, a excel file excisting of around 10.000 social messages (blogs,facebook post and twitter messages), all graded by hand into 5 categories: +2 very positive, +1 positve, 0 neutral, -1 negative, -2 very negative.  The goal of our project is to build a learning algotrm that can classify these messages by itself.

% By loading the .csv version of the excel file we where able to acces the data in Pyton. 

\subsection{Data Cleaning}
A persons tweet can only contain 140 characters, so to still be able to express yourself users mostly use slang and emoticons. Which increases the value of cleaning the messages first.
...
Facebook messages look more like twitter but, beacuse of the restriction on the size of twitter messages there is need for a short writting style that is not needed in the other 2 messages 
